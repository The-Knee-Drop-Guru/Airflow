...
def concat_data(data, s3_key, ti):
    """
    ...
    try:
        ...
        # XCom에 CSV 경로 푸시
        if ti:
            ti.xcom_push(key='binance_daily', value=OUTPUT_FILE)
            logging.info(f"Pushed concatenated data file path to XCom with key 'binance_daily'.")
        else:
